...
// Convenience macro for uncompressed firmware embedded via xxd (e.g. xxd -i file.bin > file.h)
#define RTL_FW(fw_name, fw_var, fw_len) \
    .name = fw_name, .var = fw_var, .size = (fw_len), .compressed = false, .uncompressed_size = (fw_len)
